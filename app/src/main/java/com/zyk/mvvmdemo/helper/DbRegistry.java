...
/**
 * DbRegistry class file
 * 全局数据寄存类
 * 使用DB寄存，用于寄存全局数据的DB名：mvvm_db_registry_setting，table名：setting
 * <pre>
 * 表概要：
...
public final class DbRegistry {
     ...
     * 用于寄存全局数据的DB名称
     */
    private static final String DB_NAME = "mvvm_db_registry_setting";
    ...
    private static DbRegistry sInstance;
     ...
     * 构造方法：初始化上下文环境、DB对象
     */
    private DbRegistry(Context context) {
     ...
     */
    public static DbRegistry getInstance(Context context) {
        synchronized (sInstanceLock) {
            if (sInstance == null) {
                sInstance = new DbRegistry(context);
            }
